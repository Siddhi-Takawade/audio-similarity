"""
app.py — Streamlit UI for Audio Similarity Search (FAISS + SQLite backend)
============================================================================
...
st.set_page_config(
    page_title="Audio Similarity Search — FAISS",
    page_icon="🎧",
    layout="wide",
)
...
def render_sidebar(model_type: str) -> None:
    ...
    st.sidebar.markdown("**Backend:** ⚡ FAISS + SQLite")
    st.sidebar.divider()
...
def render_info_tab() -> None:
    st.header("ℹ️ How It Works — FAISS + SQLite Backend")

    st.markdown("""
    ### Architecture

    This implementation uses **FAISS** (Facebook AI Similarity Search) for
    blazing-fast vector search and **SQLite** as a metadata + per-feature blob
    sidecar store.

    **Storage layout:**
    ```
    audio_db/
    ├── clap.index      — FAISS IndexFlatIP for CLAP vectors    (mmap-able binary)
    ├── librosa.index   — FAISS IndexFlatIP for librosa combined vectors
    └── metadata.db     — SQLite: labels, paths, hashes, per-feature BLOBs
    ```

    **Index type:** `IndexFlatIP` (exact inner product — cosine for L2-normalised
    vectors). Upgrade to `IndexIVFFlat` / `IndexIVFPQ` for 100k+ files.

    **Scoring:** 60 % CLAP (inner product) + 40 % librosa combined (re-constructed
    from FAISS via `index.reconstruct()`).
    ...
    | Pro | Con |
    |---|---|
    | Fastest ANN search available in Python | Two storage systems to manage |
    | Memory-mappable index (low RAM) | Index rebuild needed on deletion |
    | GPU acceleration available (`faiss-gpu`) | No native metadata filtering |
    | Production battle-tested (Meta) | More complex than ChromaDB |

    ```bash
    pip install faiss-cpu msclap
    # or: pip install faiss-gpu   # for CUDA acceleration
    ```
    """)
...
def main() -> None:
    ...
    st.caption("FAISS + SQLite backend · Fastest ANN search · Build a library, then find matches.")
